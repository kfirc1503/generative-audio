...
class RestorationTrainer(object):
    ...
    def log_html(self):
        if self.output_folder is not None:
            ...
            html += inspect.cleandoc(r"""
                    var el;
                    for (const key in datasets["html"]) {
                      el = document.getElementById("data_" + key + "_html");
                      if (el != null) {
                        el.innerHTML = datasets["html"][key];
                      }
                    }
                    for (const key in datasets["text"]) {
                      el = document.getElementById("data_" + key + "_text");
                      if (el != null) {
                        el.innerText = datasets["text"][key];
                      }
                    }
                    for (const key in datasets["imgs"]) {
                      el = document.getElementById("data_" + key + "_img");
                      if (el != null) {
                        el.src = "datasets:image/png;base64, " + datasets["imgs"][key];
                      }
                    }
                    for (const key in datasets["figs"]) {
                      el = document.getElementById("data_" + key + "_fig");
                      if (el != null) {
                        Plotly.newPlot(el, datasets["figs"][key]["datasets"], datasets["figs"][key]["layout"], {responsive: true});
                      }
                    }
