...
class MultiDirectionFullSubNet_Plus(FullSubNet_Plus):
    def __init__(self, config: Optional[MultiDirectionConfig] = None):
        ...
        # Modify fullband model for concatenated input
        input_size = self.num_freqs * 2  # Double for enhanced input
        self.fb_model = SequenceModel(
            input_size=input_size,
            output_size=self.num_freqs,
        ...
        self.channel_attention = ChannelTimeSenseSELayer(num_channels=2*self.num_channels, kersize=self.kersize)
        self.channel_attention_real = ChannelTimeSenseSELayer(num_channels=2*self.num_channels,
                                                              kersize=self.kersize)
        self.channel_attention_imag = ChannelTimeSenseSELayer(num_channels=2*self.num_channels,
                                                              kersize=self.kersize)

    def forward(self, noisy_mag, noisy_real, noisy_imag,
                enhanced_mag=None, enhanced_real=None, enhanced_imag=None):
        """
        Forward pass with concatenated inputs for multiple CRM outputs.
        ...
        # Process through attention with concatenated inputs
        # Magnitude features
        fb_input = torch.cat([
            self.norm(noisy_mag).reshape(batch_size, num_channels * num_freqs, num_frames),
            self.norm(enhanced_mag).reshape(batch_size, num_channels * num_freqs, num_frames)
        ], dim=1)
        fb_input = self.channel_attention(fb_input)

        # Real part features
        fbr_input = torch.cat([
            self.norm(noisy_real).reshape(batch_size, num_channels * num_freqs, num_frames),
            self.norm(enhanced_real).reshape(batch_size, num_channels * num_freqs, num_frames)
        ], dim=1)
        fbr_input = self.channel_attention_real(fbr_input)

        # Imaginary part features
        fbi_input = torch.cat([
            self.norm(noisy_imag).reshape(batch_size, num_channels * num_freqs, num_frames),
            self.norm(enhanced_imag).reshape(batch_size, num_channels * num_freqs, num_frames)
        ], dim=1)
        ...
        # Unfold outputs
        fb_output_unfolded = self.unfold(fb_output, num_neighbor=self.fb_num_neighbors)
        fb_output_unfolded = fb_output_unfolded.reshape(batch_size, num_freqs,
                                                       self.fb_num_neighbors * 2 + 1, num_frames)

        fbr_output_unfolded = self.unfold(fbr_output, num_neighbor=self.fb_num_neighbors)
        fbr_output_unfolded = fbr_output_unfolded.reshape(batch_size, num_freqs,
                                                         self.fb_num_neighbors * 2 + 1, num_frames)

        fbi_output_unfolded = self.unfold(fbi_output, num_neighbor=self.fb_num_neighbors)
        fbi_output_unfolded = fbi_output_unfolded.reshape(batch_size, num_freqs,
                                                         self.fb_num_neighbors * 2 + 1, num_frames)

        # Process subband
        noisy_mag_unfolded = self.unfold(noisy_mag, num_neighbor=self.sb_num_neighbors)
        noisy_mag_unfolded = noisy_mag_unfolded.reshape(batch_size, num_freqs,
                                                       self.sb_num_neighbors * 2 + 1, num_frames)

        # Concatenate all features
        sb_input = torch.cat([noisy_mag_unfolded, fb_output_unfolded,
                            fbr_output_unfolded, fbi_output_unfolded], dim=2)
        sb_input = self.norm(sb_input)
