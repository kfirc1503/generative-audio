...
def plot_pc_spectrograms(masked_spec, clean_spec, pred_spec_mag, pc_directions_mag,mask, sample_len_seconds,
    ...
    mask = mask.expand(mask.shape[-1] , mask.shape[-1])
    ...
    error_db = clean_mag_db - output_mag_db
    error_db_relevant_part = error_db[mask == 0]
    error_db_relevant_part = error_db_relevant_part.reshape(error_db_relevant_part.shape[-1]//error_db.shape[0],error_db.shape[0])
    im = axs[3].imshow(error_db_relevant_part.numpy(), origin='lower', aspect='auto', vmin=vmin, vmax=vmax,
    ...
    for i in range(n_dirs):
        ...
        pc_dir_db_relevant_part = pc_dir_db_relevant_part.reshape(pc_dir_db_relevant_part.shape[-1] // pc_dir_db.shape[0],
                                                                pc_dir_db.shape[0])
        im = axs[i + 4].imshow(pc_dir_db_relevant_part.cpu().numpy(), origin='lower', aspect='auto',
                               vmin=vmin, vmax=vmax,
...
class NPPCModelValidator:
    ...
    def validate_sample(self, masked_spec, mask, clean_spec, sample_len_seconds):
        ...
        with torch.no_grad():
            ...
            clean_spec_mag_log = 20*torch.log10(clean_spec_mag + 1e-6)
            ...
            pc_directions = self.model(masked_spec_mag_log, mask_broadcast)
            pred_spec_mag_log = self.model.get_pred_spec_mag_norm(masked_spec_mag_log, mask_broadcast)
            # # unnormalized:
