...
All work in this workspace uses the project virtualenv at `/storage/kfir/kfir_env/` (Python 3.12). Activate it before running anything — training, dataset generation, NPPC scripts, and one-off tools like the Grid corpus downloader:

```bash
source /storage/kfir/kfir_env/bin/activate
# or invoke the interpreter directly:
/storage/kfir/kfir_env/bin/python <script>.py
```

Do **not** rely on `/opt/conda/bin/python` (conda base) or the system `python3` — packages and versions only match in `kfir_env`.
